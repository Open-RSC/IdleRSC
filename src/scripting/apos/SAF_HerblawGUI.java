...
public final class SAF_HerblawGUI extends Script implements ActionListener {
  ...
  private final int ITEM_EYE_NEWT = 270,
  ...
  private final String[] combatChoiceStrings = {"Strength", "Attack", "Defense"},
    ...
    if (isBanking()) {
      ...
      if (potion_mode.getSelectedIndex() == 0) {
        if (bankCount(items[4]) > 0) {
          ...
          if (getInventoryCount(items[0]) < 1
              && getInventoryCount(items[1]) < 1
              && getInventoryCount(items[2]) < 1
              && getInventoryCount(items[3]) < 1
              && getInventoryCount(items[4]) < 1) {
            _end("Out of shit.");
          }
        }
      } else if (potion_mode.getSelectedIndex() == 1) {
      ...
      if (bankCount(current_item_one) < withdraw_count
          && getInventoryCount(items[0]) < 1
          && getInventoryCount(items[1]) < 1
          && getInventoryCount(items[2]) < 1
          && getInventoryCount(items[3]) < 1
          && getInventoryCount(items[4]) < 1) {
      ...
      if (count > getInventoryCount(current_item_one) && potion_mode.getSelectedIndex() != 0) {
        ...
        return random(600, 800);
      } else if (count < getInventoryCount(current_item_one)
          && potion_mode.getSelectedIndex() != 0) {
  ...
  public void paint() {
    ...
    for (int i = 0; i < xp_start.length; ++i) {
      ...
      drawString(
          SKILL[i] + " XP: " + f.format(gained) + " (" + per_hour(gained) + "/h)",
          x,
          y,
          font,
          color);
