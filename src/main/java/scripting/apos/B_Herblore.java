...
public class B_Herblore extends Script {
  ...
  public int main() {
    ...
    if (isBanking()) {
      ...
      if (!unfinished) {
        ...
        if (count > 0) {
          if (hasBankItem(potion.secondary)) {
            ...
            System.out.println(
                "Out of " + getItemNameId(potion.secondary) + " for " + potion.potion_name);
            stop(false);
          }
          closeBank();
          return 1000;
      ...
      if (!unfinished
          && count < 14
          && hasBankItem(potion.unfinished)
          && bankCount(potion.secondary) > 14) {
      ...
      count = getInventoryCount(potion.clean, potion.grimy, potion.unfinished);
      if (count < 14) {
        if (hasBankItem(potion.clean)) withdraw(potion.clean, 14 - count);
        else if (hasBankItem(potion.grimy)) withdraw(potion.grimy, 14 - count);
        else {
          System.out.println(
              "Out of "
                  + getItemNameId(potion.grimy)
                  + ", "
                  + getItemNameId(potion.clean)
                  + ", and "
                  + getItemNameId(potion.unfinished)
                  + " for "
                  + potion.potion_name);
          stop(false);
        }
  ...
  private Potion getPotion(int number) {
    ...
    potions[i] =
        new Potion(potion_names[i++], "Marrentill", 5, 37.5, 435, 3.75, 445, 473, 455, 566);
    ...
    potions[i] =
        new Potion(potion_names[i++], "Dwarf Weed", 72, 165, 443, 13.75, 453, 501, 463, 498);
  ...
  public void paint() {
    ...
    if (cleaned > 0) {
      drawString(
          potion.herb_name + " cleaned: " + cleaned + " (" + (cleaned * potion.grimy_xp) + " xp)",
          x,
          y,
          1,
          0xFFFFFF);
    ...
    if (made > 0) {
      drawString(
          potion.potion_name + " made: " + made + " (" + (made * potion.xp) + " xp)",
          x,
          y,
          1,
          0xFFFFFF);
