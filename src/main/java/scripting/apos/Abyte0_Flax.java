  ...
  public int main() {
    ...
    if (getInventoryCount(idString) == 29) {
      ...
      if (isAtApproxCoords(702, 522, 30)) {
        if (getX() < 702) walkTo(702, 522);
        else atObject(714, 516);
        return 500;
      }
      // print("lost FULL");
    ...
    // Si plein de Flax
    if (getInventoryCount(idFlax) == 29
        || getInventoryCount(idString) + getInventoryCount(idFlax) == 29) {
      if (power) {
        if (getInventoryCount(idString) > 0) dropItem(getInventoryIndex(idString));
        return random(300, 400);
      }
      // si pres des bankers
      if (isAtApproxCoords(715, 1452, 16)) {
      ...
      // si sur le chemin public
      if (isAtApproxCoords(702, 522, 30)) {
        if (getX() > 702) walkTo(701, 522);
        else atObject(692, 525);
        return 500;
      }
      return random(100, 200);
    }
