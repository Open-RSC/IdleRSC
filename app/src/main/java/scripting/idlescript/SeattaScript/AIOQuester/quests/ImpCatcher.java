...
public final class ImpCatcher extends QuestHandler {
  ...
  private final int[] WIZARD_TOWER_OUTER = {217, 687};
  private final int[] FALADOR_WEST_SOUTH = {314, 558};
  private final int[] FALADOR_WEST_NORTH = {314, 550};
  private final int[] VOLCANO_SOUTH = {418, 702};
  private final int[] VOLCANO_NORTH = {417, 681};
  private final int[] KARAMJA_DOCKS = {335, 713};
  int[] FARM_LOCATION = null;
  int[] FARM_LOCATION_ONE = null;
  int[] FARM_LOCATION_TWO = null;

  // OBJECT COORDINATES

  private final int[] WIZARD_TOWER_LADDER_UP_GROUND_FLOOR = {218, 692};
  private final int[] WIZARD_TOWER_LADDER_UP_SECOND_FLOOR = {216, 1636};
  private final int[] WIZARD_TOWER_LADDER_DOWN_THIRD_FLOOR = {216, 2580};
  private final int[] WIZARD_TOWER_LADDER_DOWN_SECOND_FLOOR = {218, 1636};

  // ITEM IDS
  private final int RED_BEAD_ID = ItemId.RED_BEAD.getId();
  private final int YELLOW_BEAD_ID = ItemId.YELLOW_BEAD.getId();
  private final int BLACK_BEAD_ID = ItemId.BLACK_BEAD.getId();
  private final int WHITE_BEAD_ID = ItemId.WHITE_BEAD.getId();
  private final int[] BEADS = {RED_BEAD_ID, YELLOW_BEAD_ID, BLACK_BEAD_ID, WHITE_BEAD_ID};
  private final int[] BEADS_TABLE = Arrays.copyOf(BEADS, BEADS.length);

  // NPC IDS
  private final int MIZGOG_ID = NpcId.WIZARD_MIZGOG.getId();
  private final int IMP_ID = NpcId.IMP.getId();
  private final int DARK_WIZARD_ID = NpcId.DARKWIZARD_LVL25.getId();
  private final int SCORPION_ID = NpcId.SCORPION.getId();
  private final int CAPTAIN_TOBIAS_ID = NpcId.CAPTAIN_TOBIAS.getId();
  private final int CUSTOM_OFFICER_ID = NpcId.CUSTOMS_OFFICER.getId();
  // NPC DIALOGS
  private final String[] MIZGOG_START_QUEST_DIALOG = {"Give me a quest!", "Give me a quest please"};
  private final String[] VOLCANO_RETURN_DIALOG = {
    "Can I board this ship?", "Search away I have nothing to hide", "Ok"
  };
  private final String[] VOLCANO_DEPART_DIALOG = {"Yes please"};

  private boolean checkForBeads() { // Check if beads are in inventory
   ...
   */
  private boolean checkLevelAggression(
