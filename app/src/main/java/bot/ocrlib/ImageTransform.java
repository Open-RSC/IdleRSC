...
class ImageTransform {
  ...
  Rectangle getMinAreaRectangle(ArrayList<Point> points) {
    ...
    for (i = 0; i < points.size() - 2; i++) {
      Rectangle tmp = getMinAreaRectangle(points.get(i), points.get(i + 1), points);
      if (tmp.area > 0) {
        if (res.area < 0) {
          res = tmp;
        } else {
    ...
    Rectangle tmp = getMinAreaRectangle(points.get(points.size() - 1), points.get(0), points);
    if (tmp.area > 0) {
      if (res.area < 0) {
        res = tmp;
      } else {
  ...
  ArrayList<Point> getConvexHull(ArrayList<Point> points) {
    ...
    while (index != points.size()) {
      if (info(result.get(result.size() - 2), result.get(result.size() - 1), points.get(index))
          > 0) {
