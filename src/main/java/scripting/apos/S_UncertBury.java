...
/**
 * Uncerts dragon bones in Yanille, buries them, sleeps in the beds above Sidney Smith. Start in her
 * house. No parameters and no sleeping bag.
 *
 * <p>Has paint, proper quest menu listener. should be able to handle things properly when less than
 * 5 certs are in the player's inventory.
 *
 * <p>Stops when very close to 99, when nothing left to do prints out runtime info.
 *
 * @author S
 */
public final class S_UncertBury extends Script {
  ...
  public void paint() {
    ...
    drawString(
        "XP gained: " + int_format(gained) + " (" + per_hour(gained) + "/h)", x, y, font, white);
    y += 15;
    drawString(
        "Bones used: " + int_format(count) + " (" + per_hour(count) + "/h)", x, y, font, white);
  }
  ...
  private int _end(String message) {
    ...
    int gained = xp - start_xp;
    System.out.println("XP gained: " + int_format(gained) + " (" + per_hour(gained) + "/h)");
    System.out.println("Bones used: " + int_format(count) + " (" + per_hour(count) + "/h)");
