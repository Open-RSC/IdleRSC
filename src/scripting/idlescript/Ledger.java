...
/**
 * Uses the ledge in Yanille agility dungeon. Eats food, runs up stairs if fail. Logs out on low hp + no food. 
 * 
 * TODOS:
 * 	if facing bat via character direction, attack it.
 * 
 * @author Dvorak
 */
public class Ledger extends IdleScript {
	...
	public void start(String[] param) {
		...
		controller.displayMessage("@yel@WARNING:@red@ this script is crappy and not death safe, but it's better than the other ledger scripts.");
...
//			if(prevX == controller.currentX() && prevY == controller.currentY()) //detect and attack bats which block us 
//				killBat();
//			
...
//			prevY = controller.currentY();
	...
	private void bank() {
		if(controller.currentY() <= 735) {
			...
			while(controller.getInventoryItemCount() != 30) {
				...
				controller.walkPathReverse(new int[] {
						603, 725,
						603, 730,
						594, 734, 
						589, 736,
	...
	private void killBlocks() {
		if(controller.currentX() == 601 && controller.currentY() == 3557)
			return;
...
//			if(controller.distance(controller.currentX(), controller.currentY(), coords[0], coords[1]) <= 1) {
//				controller.setStatus("@yel@Swatting bat away...");
...
//		}
	...
	private boolean eatFood() {
		if(controller.getCurrentStat(controller.getStatId("Hits")) <= (controller.getBaseStat(controller.getStatId("Hits")) / 2)) {
...
//	}
    ...
    public void serverMessageInterrupt(String message) {
        if(message.contains("You skillfully balance"))
        	successCount++;
    }
    
    @Override
    public void questMessageInterrupt(String message) {
        if(message.contains("you lose your footing"))
        	failureCount++;
    }
    ...
    public void paintInterrupt() {
        if(controller != null) {
            ...
            controller.drawString("@yel@Successes: @whi@" + String.format("%,d", successCount) + " @yel@(@whi@" + String.format("%,d", successesPerHr) + "@yel@/@whi@hr@yel@)", 10, 21+14, 0xFFFFFF, 1);
            controller.drawString("@yel@Failures: @whi@" + String.format("%,d", failureCount) + " @yel@(@whi@" + String.format("%,d", failuresPerHr) + "@yel@/@whi@hr@yel@)", 10, 21+14+14, 0xFFFFFF, 1);
        }
    }
