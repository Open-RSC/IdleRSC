...
public class J_Quester extends Script implements ActionListener {
  ...
  private final class Barcrawl extends Quests {
    ...
    public int doQuest() {
      ...
      if (!quest_init) {
        ...
        for (int i = 0; i < items.length; i++) {
          ...
          if (!check_inventory(items[i]) || getInventoryCount(items[i]) < qty[i]) {
            System.out.println(
                "You're missing following required item: "
                    + qty[i]
                    + "x "
      ...
      if (isQuestMenu()) {
        ...
        if (questMenuCount() >= 2 && questMenuOptions()[1].contains("Looks can be deceiving")
            || questMenuOptions()[1].contains("I'm doing Alfred Grimhand's barcrawl")) {
          answer(1);
        ...
        if (questMenuCount() >= 3
            && questMenuOptions()[2].contains("I'm doing Alfred Grimhand's barcrawl")) {
          seers_card = true;
        ...
        if (questMenuCount() >= 4
            && questMenuOptions()[3].contains("I'm doing Alfred Grimhand's barcrawl")) {
          brim_card = true;
      ...
      if (in_dungeon && !out_dungeon) {
        if (passed_door && !passed_door2) {
          if (!dipped) {
            ...
            if (DR_check_meat(E_RAT_MEAT_ID) == 1
                && DR_check_meat(E_BEAR_MEAT_ID) == 1
                && DR_check_meat(E_CHICKEN_MEAT_ID) == 1
                && DR_check_meat(E_COW_MEAT_ID) == 1) {
              dipped = true;
            } else {
              if (DR_check_meat(RAT_MEAT_ID) == 1) {
          ...
          } else if (passed_door2
              && isAtApproxCoords(DUNGEON_PATH[step], DUNGEON_PATH[step + 1], 0)) {
            DR_climb_up();
            out_dungeon = true;
            return random(2000, 3000);
          } else {
  ...
  private void walk_approx(int x, int y, int range) {
    ...
    do {
      ...
      if ((++loop) > 1000) return;
    } while ((dx == getX() && dy == getY()) || !isReachable(dx, dy));
    walkTo(dx, dy);
  }
