...
public class SmithGearSet extends IdleScript {
		...
		while (controller.isRunning()) {
			while (bankTime && !controller.isInBank()) {
				...
				while (controller.getInventoryItemCount() > 1 && controller.isInBank()) {
					...
					totalLegs = totalLegs + controller.getInventoryItemCount(itemType[barField.getSelectedIndex()][2]);
					totalKites = totalKites
							+ controller.getInventoryItemCount(itemType[barField.getSelectedIndex()][3]);
					totalBaxes = totalBaxes
							+ controller.getInventoryItemCount(itemType[barField.getSelectedIndex()][4]);
					total2Hs = total2Hs + controller.getInventoryItemCount(itemType[barField.getSelectedIndex()][5]);
					for (int itemId : controller.getInventoryItemIds()) {
						if (itemId != 168 && itemId != 1263) {
							controller.depositItem(itemId, controller.getInventoryItemCount(itemId));
						}
					}
					totalSets = ((controller.getBankItemCount(itemType[barField.getSelectedIndex()][0])
							...
							+ controller.getBankItemCount(itemType[barField.getSelectedIndex()][5])) / 6);
	...
	public void paintInterrupt() {
		if (controller != null) {
			...
			controller.drawString("@red@Helms Smithed: @yel@" + String.valueOf(this.totalHelms), 10, 35, 0xFFFFFF, 1);
			controller.drawString("@red@Plates Smithed: @yel@" + String.valueOf(this.totalPlates), 10, 49, 0xFFFFFF, 1);
			controller.drawString("@red@Legs Smithed: @yel@" + String.valueOf(this.totalLegs), 10, 63, 0xFFFFFF, 1);
			controller.drawString("@red@Kites Smithed: @yel@" + String.valueOf(this.totalKites), 10, 77, 0xFFFFFF, 1);
			controller.drawString("@red@Baxes Smithed: @yel@" + String.valueOf(this.totalBaxes), 10, 91, 0xFFFFFF, 1);
			controller.drawString("@red@2Hs Smithed: @yel@" + String.valueOf(this.total2Hs), 10, 105, 0xFFFFFF, 1);
			controller.drawString("@red@Sets in Bank: @yel@" + String.valueOf(this.totalSets), 10, 105 + 14, 0xFFFFFF,
					1);
		}
	}
}