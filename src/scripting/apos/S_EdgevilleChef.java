...
public final class S_EdgevilleChef extends Script implements ActionListener, ItemListener {
  ...
  public int main() {
    ...
    if (raw != -1) {
      ...
      // midway
      if (getWallObjectIdFromCoords(X_SHOP_DOOR, Y_SHOP_DOOR) == ID_SHOP_SHUT) {
  ...
  public void actionPerformed(ActionEvent event) {
    if (event.getActionCommand().equals("OK")) {
      try {
        ...
        System.out.println("Should be numerical, separated with \",\".");
        return;
      }
      banked_count = 0;
    }
  ...
  private static boolean inArea(int myx, int myy, int x1, int y1, int x2, int y2) {
