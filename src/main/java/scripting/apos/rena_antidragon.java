...
public class rena_antidragon extends Script {
  ...
  int[] intervals = new int[] {5, 11, 17, 23, 29};
  ...
  public int main() {
    ...
    if (draytoduke && !demandshields && !jugglingshields && !duketodray && !banking) {
      ...
      if (isAtApproxCoords(219, 638, 3)) {
        ...
        path =
            new int[] {
              219, 633, 214, 633, 206, 644, 195, 643, 184, 643, 174, 643, 160, 643, 144, 643, 128,
              645, 116, 657, 129, 659, 138, 666
            };
      }
      /*
      else
    ...
    if (!draytoduke && demandshields && !jugglingshields && !duketodray && !banking) {
      ...
      if (shieldcount == 35) // 30 Shields on the floor, lets pick them
      {
        ...
        if (demshields[0] != -1) {
          ...
          System.out.println(
              "[Rena] Picking up dem shields at " + demshields[1] + "," + demshields[2] + " ...");
          System.out.println(" ");
    ...
    if (!draytoduke && !demandshields && !jugglingshields && duketodray && !banking) {
      ...
      if (isAtApproxCoords(138, 666, 1)) {
        ...
        path =
            new int[] {
              129, 659, 116, 657, 128, 645, 144, 643, 160, 643, 174, 643, 184, 643, 195, 643, 206,
              644, 214, 633, 219, 633, 219, 636
            };
      }
      if (isAtApproxCoords(219, 638, 3)) {
        draytoduke = false;
    ...
    if ((step + 1) < path.length) {
      ...
      if (isAtApproxCoords(path[step], path[step + 1], 0)) step = step + 2;
