...
public final class S_FishingTrawler extends Script implements ActionListener {
  ...
  private static final int[] trawler_nets = {1102, 1101};

  private static final int[] leaks = {1071, 1077};
  ...
  public S_FishingTrawler(String ex) {
    //        super(ex);
    ...
    if (isQuestMenu()) {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        if (str.contains("access")) {
          ...
          return random(1000, 2000);
        } else if (str.contains("could i help")
            || str.contains("be fine")
            || str.contains("do it")) {
          answer(i);
    ...
    for (int i = 0; i < trawler_nets.length; ++i) {
      ...
      if (net[0] != -1) {
        ...
        if (rope != -1) {
          ...
          tnets_checked[i] =
              System.currentTimeMillis() + random(net_check_min_ms, net_check_max_ms);
          return true;
        } else {
          return false;
        }
  ...
  public void paint() {
    ...
    for (TrawlerLoot l : loot) {
      ...
      drawString("Banked " + l.name + ": " + l.banked_count, 25, y, 1, 0xFFFFFF);
      y += 15;
    }
  }
