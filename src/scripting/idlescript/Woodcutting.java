...
public class Woodcutting extends IdleScript {
    ...
    JComboBox<String> destination = new JComboBox<String>(new String[] {"Draynor","Varrock West","Varrock East","Draynor","EdgeVille","Falador","Seers","North Ardy","South Ardy","Yanille"});
	...
	int bankSelX = -1;
	int bankSelY = -1;
	int[] bankX = {220,150,103,220,216,283,500,582,566,588};
	int[] bankY = {635,504,511,365,450,569,451,576,600,754};
	...
	public void startWalking(int x, int y) {
		...
		while(controller.currentX() != x && controller.currentZ() != y) {
			if(controller.currentX()-x > 23) {newX = controller.currentX()-20;}
			if(controller.currentZ()-y > 23) {newY = controller.currentZ()-20;}
			if(controller.currentX()-x < -23) {newX = controller.currentX()+20;}
			if(controller.currentZ()-y < -23) {newY = controller.currentZ()+20;}
			if(Math.abs(controller.currentX()-x) <= 23) {newX = x;}
			if(Math.abs(controller.currentZ()-y) <= 23) {newY = y;}
				if(!controller.isTileEmpty(newX,newY)) {
					controller.walkTo(newX, newY, 2, false);
				}else {controller.walkTo(newX, newY, 0, false);}
		}
	}
	...
	public void scriptStart() {
		if (controller.getInventoryItemCount() == 30) { bankTime = true; chopTime = false;}
		if (controller.getInventoryItemCount() <= 29) { bankTime = false;}
        ...
        if (controller.getInventoryItemCount() >= 0 && controller.getNearestNpcByIds(bankerIds, false) != null && bankTime && !controller.isInBank()) {
        	controller.npcCommand1(controller.getNearestNpcByIds(bankerIds, false).serverIndex);
    	...
    	if(controller.currentX() == saveX && controller.currentZ() == saveY && !chopTime){
	...
	public void setupGUI() { 	
    	...
    	JComboBox<String> treeField = new JComboBox<String>(new String[] {"Normal", "Oak", "Willow", "Maple", "Yew", "Magic"});
