...
import patcher.utils.FileUtils;
...
public class JARManipulator {
  ...
  public static void compress(String sourceDirectory, String destinationArchive)
    ...
    ValidationUtils.validateDirectory(directory);
          ...
          .forEach(
              path -> {
                try {
                  ...
                  throw new RuntimeException("Failed to compress file: " + path, e);
                }
              });
    }
  }
  ...
  public static void decompress(String archiveFile, String destinationDirectory)
    ...
    ValidationUtils.validateJarFile(archive);

    File destDir = new File(destinationDirectory);
    FileUtils.ensureDirectoryExists(destinationDirectory);

    byte[] buffer = new byte[BUFFER_SIZE];
    try (ZipInputStream zin = new ZipInputStream(Files.newInputStream(Paths.get(archiveFile)))) {
