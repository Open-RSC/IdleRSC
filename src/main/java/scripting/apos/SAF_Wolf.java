...
public class SAF_Wolf extends Script {
  ...
  public int main() {
    ...
    if (getFatigue() < 99) {
      ...
      if (getFatigue() < 97 && groundItem[0] != -1) {
        if (groundItem[1] > 740
            && groundItem[2] > 539
            && !isObjectAt(groundItem[1], groundItem[2])) {
          useItemOnGroundItem(
  ...
  public void paint() {
    ...
    int b =
        (int) ((((a - start_xp) * 60L) * 60L) / ((System.currentTimeMillis() - this.time) / 1000L));
    drawString(
        "xp gained: @gre@" + (a - start_xp) + "@whi@ (@gre@" + b + "@whi@ /h)", x, y, 1, 0xFFFFFF);
    y += 15;
