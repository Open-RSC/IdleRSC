...
public final class S_OgreFatigueBones extends Script {
  ...
  private long bank_time, menu_time, start_time;
  private int start_xp;
  private final DecimalFormat iformat = new DecimalFormat("#,##0");

  public S_OgreFatigueBones(String ex) {
    //		super(ex);
  ...
  public void init(String params) {
    ...
    String[] food_types = {"Lobster", "Swordfish", "Shark"};
    Object sfood =
    ...
    to_bank =
        pw.calcPath(
            617 + random(-3, 3), 780 + random(-3, 3), 587 + random(-2, 2), 754 + random(-1, 1));

    from_bank =
        pw.calcPath(
            587 + random(-2, 2), 754 + random(-1, 1), 617 + random(-3, 3), 780 + random(-3, 3));
  }
  ...
  public void paint() {
    ...
    drawString(
        String.format("Prayer XP gained: %s (%s/h)", iformat.format(gained), per_hour(gained)),
        x,
        y,
        2,
        0xFFFFFF);
  ...
  private boolean inside_bank() {
    return getX() <= 590 && getX() >= 585 && getY() >= 750 && getY() <= 758;
  }
