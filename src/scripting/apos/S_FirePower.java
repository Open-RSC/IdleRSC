...
public final class S_FirePower extends Script {
  ...
  private static final int LOGS = 14, TINDERBOX = 166, WOODCUT = 8, FIREMAKING = 11;

  private static final int[] axe_ids = {12, 87, 88, 203, 204, 405, 428};

  private static final int[] ids_tree = {0, 1, 70};
  ...
  public int main() {
    ...
    if (getLevel(FIREMAKING) < 99 && (fire_xp + 200) >= 13034431) {
      return _end(
          "Stopping to let you take over so you can get 99 firemaking at a party, or something.");
    }
    if (getLevel(WOODCUT) < 99 && (wood_xp + 30) >= 13034431) {
      return _end(
          "Stopping to let you take over so you can get 99 woodcut at a party, or something.");
    }
    if (inCombat()) {
      pw.resetWait();
  ...
  public void paint() {
    ...
    drawString(
        "Firemaking XP: " + int_format(gained) + " (" + per_hour(gained) + "/h)",
        x,
        y,
        font,
        white);
    ...
    drawString(
        "Woodcutting XP: " + int_format(gained) + " (" + per_hour(gained) + "/h)",
        x,
        y,
        font,
        white);
