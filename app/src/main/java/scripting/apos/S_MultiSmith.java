...
public final class S_MultiSmith extends Script implements ActionListener {
  ...
  private static final Point[] varrock_anvil_points = {new Point(148, 512), new Point(145, 511)};

  private static final int[] bank_ids = {
    0, 1, 2, 3, 5, 6, 7, 8, 9, 11, 12, 28, 31, 32, 33, 34, 35, 36, 37, 38, 40, 41, 42, 46, 62, 63,
    64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82, 83, 84, 85, 86, 87,
    88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113,
    114, 115, 116, 117, 118, 119, 120, 121, 122, 123, 124, 125, 126, 127, 128, 129, 130, 131, 151,
    153, 154, 156, 203, 204, 205, 206, 214, 215, 225, 226, 227, 242, 308, 309, 310, 311, 312, 396,
    397, 398, 399, 400, 401, 402, 403, 404, 405, 406, 407, 419, 517, 519, 559, 560, 561, 562, 563,
    564, 574, 619, 638, 639, 640, 641, 642, 643, 644, 645, 646, 647, 669, 670, 671, 672, 673, 674,
    698, 796, 803, 825, 827, 975, 979, 1013, 1015, 1024, 1062, 1063, 1064, 1065, 1066, 1067, 1068,
    1069, 1070, 1075, 1076, 1077, 1078, 1079, 1080, 1088, 1089, 1090, 1091, 1092, 1097, 1122, 1123,
    1124, 1125, 1126, 1127, 1128, 1129, 1130, 1131, 1133, 1134, 1135, 1136, 1137, 1138, 1139, 1140,
  ...
  public void paint() {
    ...
    drawString(
        "XP gained: " + iformat.format(xp_gained) + " (" + _perHour(xp_gained) + "/h)",
        x,
        y,
        2,
        white);
    y += 15;
    drawString(
        "Bars used: " + iformat.format(bars_used) + " (" + _perHour(bars_used) + "/h)",
        x,
        y,
        2,
        white);
    ...
    for (int i = 0; i < list_sz; ++i) {
      ...
      if (count <= 0) continue;
      drawString(
          s.last_option + ": " + iformat.format(count) + " (" + _perHour(count) + "/h)",
          x,
          y,
          2,
          ptr == i ? red : white);
  ...
  private String _getRuntime() {
    ...
    if (secs >= 3600) {
      return iformat.format((secs / 3600))
          + " hours, "
          + ((secs % 3600) / 60)
          + " mins, "
          + (secs % 60)
