...
public final class TheRestlessGhost extends QuestHandler {
  ...
  private final int[] SKULL = {218, 3521};
  private final int[] LADDER_UP = {218, 3526};
  private final int[] COFFIN = {103, 675};
  // OBJECT COORDINATES

  private final int[] CHURCH_DOOR = {115, 664};
  private final int[] GRAVEYARD_DOOR = {106, 675};
  private final int[] SWAMP_DOOR = {116, 709};

  private final int[] APOTHECARY_DOOR = {216, 2580};

  // ITEM IDS
  private final int GHOSTSPEAK_ID = ItemId.AMULET_OF_GHOSTSPEAK.getId();
  private final int SKULL_ID = ItemId.QUEST_SKULL.getId();

  // NPC IDS
  private final int PRIEST_ID = NpcId.PRIEST.getId();
  private final int URHNEY_ID = NpcId.URHNEY.getId();
  private final int GHOST_ID = NpcId.GHOST_RESTLESS.getId();
  private final int APOTHECARY_ID = NpcId.APOTHECARY.getId();

  // NPC DIALOGS
  private final String[] STEP_ONE_START_QUEST_DIALOG = {"I'm looking for a quest"};
  private final String[] STEP_TWO_QUEST_DIALOG = {
    "Father Aereck sent me to talk to you", "He's got a ghost haunting his graveyard"
  };
  private final String[] STEP_THREE_QUEST_DIALOG = {"Yep, now tell me what the problem is"};
