...
/**
 * Taverly Blue Dragons, Black Demons, Black Dragons, or King Black Dragons in wild - Picks up the
 * loot : rune battleaxe rune 2-h rune long rune chain rune med rune kite rune sq dmed d sq half
 * half keys chaos, deaths, blood, fire runes dragon stones dragon bones (buries them also) - teles
 * falador to bank. - option to use ss - option to use food or pray pots
 *
 * <p>shauder notes: start by the west bank in fally or the ladder to the dungeon
 *
 * @author shauder with much of it stolen from @Storms S_TravBlackDemons script (pretty much all of
 *     it)
 */
public final class SAF_Fally_DnD extends Script implements ActionListener {

  // user settings. edited from GUI
  private int eat_at = 50,
      pray_at = 20,
      food_count = 2,
      ppot_count = 10,
      food_id = 546, /* shark */
  ...
  private final Checkbox drink_ss = new Checkbox("Use super sets", true),
  ...
  private static final int[] cp_pots = {568, 567, 566};
  ...
  private static final int[] base_items = {SLEEPING_BAG};

  private static final int[] equipt_items = {388, 389, 420, 597, 594, 795};
  ...
  private static final int[] rare_drops = {795, 1277};
  // stuff also in pickup_ids that shouldn't be banked
  private static final int[] dont_bank = {32, AIR_RUNE, LAW_RUNE};
  ...
  public SAF_Fally_DnD(String ex) {
    //        super(ex);
  ...
  public int main() {
    ...
    if (isBanking()) {
      ...
      if (target_npc == BLACK_DRAGON
              && getInventoryCount(cp_pots[0]) < 1
              && getInventoryCount(cp_pots[1]) < 1
              && getInventoryCount(cp_pots[2]) < 1
          || target_npc == KBD
              && getInventoryCount(cp_pots[0]) < 1
              && getInventoryCount(cp_pots[1]) < 1
              && getInventoryCount(cp_pots[2]) < 1) {
    ...
    if (food_index != -1 && food_count > 0 && getCurrentLevel(SKILL_HP) <= eat_at
        || in_fight_area() && !inCombat() && getCurrentLevel(SKILL_HP) <= getLevel(SKILL_HP) - 20) {
      useItem(food_index);
      return random(600, 800);
    }
    ...
    if (is_underground()) {
      ...
      if (!inCombat()
          && isPrayerEnabled(PARA_MONSTER)
          && npc[1] != getX()
          && npc[2] != getY()
          && (System.currentTimeMillis() - last_moved) >= 1000) {
        disablePrayer(PARA_MONSTER);
      }
