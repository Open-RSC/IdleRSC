...
public class GuildFisher2 extends Script implements ActionListener {
  ...
  public void ServerMessage(String message) {
    if (message.equals("@gam@You are too tired to catch this fish")
        || message.equals("@gre@You are too tired to gain experience, get some rest")) {
      sleep = true;
    ...
    while (Running()) {
      ...
      if (InvCount() == 30 && Running()) {
        ...
        if (FM.equalsIgnoreCase("Fish then cook then cert")) {
          ...
          if (Running()) cMode = " walking back to the pier";
          showReport();
          Walk(589, 502);
          Wait(3000);
        }
  ...
  public void LastTime() {
    ...
    int line = 0;
    try {
      FileInputStream fstream = new FileInputStream("guildfisher.txt");
