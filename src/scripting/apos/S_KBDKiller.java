...
/**
 * KBD Killer - no world hopping - Uses super sets. - Full gear - Switches anti-breath shield to
 * rune kite when been in combat for 2 sec. - If kills the dragon and has less or equal to 5 foods
 * left, will fall back to lever. - Trades loot over to the loot-guys, which ever is available and
 * takes sharks in return.
 *
 * <p>just so we're in same page. i'd love all of the chars to be in w2 when they are in the kbd
 * room. so when the looters reach there, they cant be like.. "oh im in wrong world, changing" if
 * you know what i mean lol. if someone else comes to w2 when they are in the kbd room it dont
 * matter
 *
 * @author Storm
 */
public final class S_KBDKiller extends Script implements ActionListener {
  ...
  private static final int SHARKS_TO_RUN = 2,
      SUPER_ATT = 486,
      SUPER_DEF = 495,
      SUPER_STR = 492,
      SLEEPING_BAG = 1263,
  ...
  private static final int[] dstone_ammies = {DSTONE_CHARGED, 522};
  ...
  private static final int[] att_pots = {486, 487, 488};

  private static final int[] def_pots = {495, 496, 497};

  private static final int[] str_pots = {492, 493, 494};

  private static final int[] poison_pots = {569, 570, 571, 566, 567, 568};
  ...
  public void init(String params) {
    if (frame == null) {
      ...
      frame.add(
          new Label(
              "Equipment may include shield_id, must include anti-dragon shield ("
                  + ANTI_DRAG_SHIELD
                  + ")."));
      frame.add(
          new Label(
              "If an extra one is not desired Shield ID must not be in equipment and shield_id"));
      frame.add(
          new Label(
              "must be -1. Equipment may also may include sleeping bag. (" + SLEEPING_BAG + ")"));
  ...
  public int main() {
    ...
    if (inCombat()) {
      ...
      if (!in_fight_area() || getCurrentLevel(HITS) <= eat_at || getInventoryIndex(SHARK) == -1) {
        walkTo(getX(), getY());
    ...
    if (isBanking()) {
      ...
      int shark_count =
          (traders.length <= 0) ? getEmptySlots() : (MIN_SHARKS - getInventoryCount(SHARK));
      if (shark_count > 0) {
    ...
    if (in_spider_area()) {
      ...
      if ((traders.length > 0 && getInventoryIndex(dstone_ammies) == -1)
          || (traders.length <= 0 && !should_bank())) {
  ...
  public void paint() {
    ...
    if (autobury.getState()) {
      ...
      drawString(
          "Prayer XP: " + int_format(gained) + " (" + per_hour(gained) + "/h)", x, y, font, white);
      y += 15;
    }
    x += 10;
    ...
    for (int i = 0; i < loot.length; ++i) {
      ...
      drawString(
          banked_counts[i]
              + " "
              + getItemNameId(loot[i])
              + " ("
              + per_hour(banked_counts[i])
              + "/h)",
          x,
          y,
          font,
  ...
  private String get_runtime() {
    ...
    if (secs >= 60L) {
      return secs / 60L + " mins, " + (secs % 60L) + " secs.";
    }
    return secs + " secs.";
  }
