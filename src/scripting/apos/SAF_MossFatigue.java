...
public final class SAF_MossFatigue extends Script implements ActionListener {
  ...
  private final int TINDERBOX = 166,
      ...
      NATURE_RUNE = 40,
      BLOOD_RUNE = 619;
  private final int[] MOSS = {626, 507},
      BANK = {580, 573},
      TREES = {0, 1},
      DAMAGE = {593, 597, 1006},
  ...
  public SAF_MossFatigue(String e) {
    //		super(e);
  ...
  public int main() {
    ...
    if (inCombat() && getAccurateFatigue() <= 98.92
        || inCombat() && getAccurateFatigue() == 100.00) {
      if (!wtf_mode) {
        wtf_mode = true;
      } else {
        for (int item : ATTACK_ITEMS) {
    ...
    if (getAccurateFatigue() <= 98.92) {
      ...
      if (getAccurateFatigue() <= 95.84 && groundItem[0] != -1) {
        if (groundItem[1] < 629 && !isObjectAt(groundItem[1], groundItem[2])) {
          useItemOnGroundItem(
              getInventoryIndex(TINDERBOX), groundItem[0], groundItem[1], groundItem[2]);
          return 1000;
        }
      }
  ...
  public void paint() {
    ...
    if (((System.currentTimeMillis() - this.time) / 1000L) != 0) {
      int b =
          (int)
              ((((a - start_xp) * 60L) * 60L) / ((System.currentTimeMillis() - this.time) / 1000L));
