...
public final class S_DummyTrainer extends Script {
  ...
  public void paint() {
    ...
    drawString(
        String.format("XP gained: %s (%s/h)", iformat.format(gained), per_hour(gained)),
        x,
        y,
        1,
        0xFFFFFF);
    ...
    drawString(
        String.format(
            "%s withdrawn: %s (%s/h)",
            drink.name, iformat.format(withdrawn_count), per_hour(withdrawn_count)),
        x,
        y,
        1,
        0xFFFFFF);
  ...
  public void onServerMessage(String str) {
    ...
    } else if (str.contains("hit the dummy") || str.contains("nothing more")) {
      click_time = -1L;
    }
  }
  ...
  private int inside_training_room() {
    ...
    if (getCurrentLevel(ATTACK) < 8) {
      ...
      if (dummy[0] != -1 && !isWalking() && current_time > (click_time + 8000L)) {
        atObject(dummy[1], dummy[2]);
        click_time = current_time;
        return random(110, 150);
      }
  ...
  private void walk_to_unused_dummy() {
    for (Point p : dummies) {
      ...
      for (int i = 0; i < count; ++i) {
        if (getPlayerX(i) == p.x && getPlayerY(i) == p.y) {
          taken = true;
          break;
        }
      }
