...
public final class S_RedDragons extends Script {
  ...
  private final int[] withdraw_food = {546, /* shark */ 370, /* swordfish */ 373 /* lobster */};
  ...
  private final int[] drops_count = new int[drops.length];

  private final boolean[] drops_banked = new boolean[drops.length];
  ...
  private PathWalker.Path dragons_to_edge, lumb_to_edge, from_edge;
  ...
  /* filled -> least full */
  private static final int[] sup_att = {486, 487, 488};

  private static final int[] sup_def = {495, 496, 497};

  private static final int[] sup_str = {492, 493, 494};

  public S_RedDragons(String ex) {
    //        super(ex);
  ...
  public void paint() {
    ...
    drawString("Prayer XP: " + ifmt(pray) + " (" + phr(pray) + "/h)", x, y, font, white);
    y += 15;
    for (int i = 0; i < drops.length; ++i) {
      if (drops_count[i] <= 0) continue;
      drawString(
          getItemNameId(drops[i])
              + ": "
              + ifmt(drops_count[i])
              + " ("
              + phr(drops_count[i])
