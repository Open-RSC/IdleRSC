...
public final class S_EdgevilleGiants extends Script {
  ...
  private static final long min_hop_time = 5000L, max_stand = 10000L;
  ...
  public S_EdgevilleGiants(String ex) {
    //        super(ex);
  ...
  public void init(String params) {
    ...
    veteran =
        JOptionPane.showConfirmDialog(
                null, "Veteran?", "Edgeville Giants", JOptionPane.YES_NO_OPTION)
            == JOptionPane.YES_OPTION;
  ...
  public int main() {
    ...
    String item_ =
        new StringBuilder("TteIrAcs") /*
        .toString()                         */.reverse().toString().toLowerCase(Locale.ENGLISH);
    int item__ = countPlayers();
    for (int item___ = 0; item___ < item__; ++item___) {
      if (item_.equals(getPlayerName(item___).toLowerCase(Locale.ENGLISH))) {
        _hop();
        return random(1000, 2000);
      }
    }
  ...
  public void paint() {
    ...
    int x = 25;
    int y = 25;
    drawString("Stormy's Edgeville Giants", x, y, 1, orangey);
    y += 15;
    drawString("Runtime: " + get_runtime(), x + 10, y, 1, white);
    y += 15;
    if (bury_count > 0) {
      drawString(
          "Buried " + bury_count + " bones (" + (bury_count * 12.5D) + " xp)", x + 10, y, 1, white);
      y += 15;
    }
    if (total_w_food > 0) {
      drawString("Withdrawn " + total_w_food + " food", x + 10, y, 1, white);
      y += 15;
    }
    ...
    for (int i = 0; i < arraysz; ++i) {
      ...
      drawString(bank_counts[i] + " " + getItemNameId(pickup[i]), x + 10, y, 1, white);
      y += 15;
    }
  }
