...
public final class WitchsPotion extends QuestHandler {
  ...
  // OBJECT COORDINATES
  private final int[] CAULDRON = {316, 666};

  private final int[][] RIMMINGTON_DOORS = {
    {319, 667}, // door one
    {324, 667}, // door two
    {329, 661}, // door three
    {320, 662} // door four
  ...
  // ITEM IDS
  private final int COOKED_MEAT_ID = ItemId.COOKEDMEAT.getId();
  private final int BURNT_MEAT_ID = ItemId.BURNTMEAT.getId(); // 2gp at The Clothes Shop
  private final int EYE_OF_NEWT_ID = ItemId.EYE_OF_NEWT.getId();
  private final int ONION_ID = ItemId.ONION.getId(); //
  private final int RATS_TAIL_ID = ItemId.RATS_TAIL.getId(); //
  private final int COIN_ID = ItemId.COINS.getId(); // // 1 gp Lumbridge General Store
  private final int[] REQUIRED_ITEMS = {
    COOKED_MEAT_ID, EYE_OF_NEWT_ID, ONION_ID, RATS_TAIL_ID, COIN_ID
  };
  private final int[][] drops = {{RATS_TAIL_ID, 1}};

  // NPC IDS
  private final int HETTY_ID = NpcId.HETTY.getId();
  private final int RAT_ID = NpcId.RAT_WITCHES_POTION.getId();
  ...
  private void depositNonQuestItems(int gold_amount_needed) {
    c.walkTowardsBank();
    c.openBank();
    c.sleep(GAME_TICK * 4);
    c.depositAll();
  ...
  private void pickOnions(int amount) {
  ...
  private void farmNpcForDrops(int npcID, int[][] drops, int startingX, int startingY) {
