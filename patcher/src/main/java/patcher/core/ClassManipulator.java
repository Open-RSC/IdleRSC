...
public class ClassManipulator {
  ...
  public void patchClass(String absoluteClassPath) throws IOException {
    ...
    ClassReader reader = new ClassReader(Files.newInputStream(Paths.get(classPath)));
    ClassWriter writer = new ClassWriter(reader, ClassWriter.COMPUTE_FRAMES);
    ClassVisitor visitor = writer;

    // First apply class-level modifications if configured. A class can have more than
    // one class-level adapter registered (e.g. orsc/mudclient has both StableCoordAdapter
    // and BubbleLimitAdapter), so every entry needs to wrap the chain in turn rather than
    // picking a single adapter.
    if (AdapterConfig.hasClassAdapter(className)) {
      for (String adapterClassName : AdapterConfig.getClassAdapters(className)) {
  ...
  public static byte[] manipulateClass(InputStream classFileStream, String className)
    ...
    // First apply class-level modifications if configured. Same as patchClass: a class
    // may have multiple class-level adapters registered, so apply all of them.
    if (AdapterConfig.hasClassAdapter(className)) {
      for (String adapterClassName : AdapterConfig.getClassAdapters(className)) {
        PatchLogger.logDebug("Found adapter: " + adapterClassName);
        try {
