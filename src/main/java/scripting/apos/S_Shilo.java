...
public final class S_Shilo extends Script {
        ...
        switch (stages.get(stage).id) {
          ...
          case STAGE_SELL:
            ...
            if (npc[0] == FERNAHEI) {
              ...
              move_y = FERNAHEI_SHOP_Y + random(0, 4);
            } else if (npc[0] == OBLI) {
              move_x = OBLI_SHOP_X + random(-1, 1);
              move_y = OBLI_SHOP_Y - random(0, 3);
            }
            break;
          default:
  ...
  public void paint() {
    ...
    drawString("Shilo Script - @whi@" + stages.get(stage).toString(), x, y, 2, orangey);
    y += 15;
  ...
  private void create_frame() {
    ...
    final Checkbox buy_raw =
        new Checkbox("Buy raw fish (Fernahei's Fishing Shop)", acquire_group, false);
    acquire_boxes.add(buy_raw);

    final Checkbox buy_cooked =
        new Checkbox("Buy cooked fish (Obli's General Store)", acquire_group, false);
    ...
    final Checkbox sell_cooked =
        new Checkbox("Sell cooked fish (Obli's General Store)", dispose_group, false);
    dispose_boxes.add(sell_cooked);
    sell_cooked.setEnabled(false);

    final Checkbox sell_raw =
        new Checkbox("Sell raw fish (Fernahei's Fishing Shop)", dispose_group, false);
        ...
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            ...
            if (deposit.getState()) {
              ...
              stages.add(new Stage(STAGE_SELL, calc_path(x, y, FERNAHEI_SHOP_X, FERNAHEI_SHOP_Y)));
              stages.get(0).path_to =
                  calc_path(
                      FERNAHEI_SHOP_X, FERNAHEI_SHOP_Y,
                      start_x, start_y);
            } else if (sell_cooked.getState()) {
              stages.add(new Stage(STAGE_SELL, calc_path(x, y, OBLI_SHOP_X, OBLI_SHOP_Y)));
              stages.get(0).path_to =
                  calc_path(
