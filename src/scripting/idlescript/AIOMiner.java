...
public class AIOMiner extends IdleScript {
    ...
    public void paintInterrupt() {
        if (controller != null) {
            ...
            controller.drawString("@red@Ores mined: @whi@" + String.format("%,d", this.oresMined) + " @red@(@whi@"
                    + String.format("%,d", minedPerHr) + "@red@/@whi@hr@red@)", 10, 21 + 14, 0xFFFFFF, 1);
            int oresInBank = 0;
            controller.drawString("@red@Ores in bank: @whi@" + String.format("%,d", oresInBank), 10, 21 + 14 + 14, 0xFFFFFF, 1);
        }
    }
    ...
    public void run() {
        ...
        Predicate<Boolean> isAtBank = c -> getBotController().playerApi.getCurrentLocation()
                .equals(getBotController().bankApi.getNearestBankPoint());
        Predicate<Boolean> isAtCamp =
                c -> distance(getBotController().playerApi.getCurrentLocation(), camp) < CAMP_DISTANCE_THRESHOLD;
        Predicate<Boolean> isRocksPresent =
                c -> getBotController().environmentApi.getNearestInteractable(scriptOptions.rocksToMine).isPresent();
    ...
    private void setupGUI() {
        ...
        scriptFrame.add(new JLabel("Changing mining camp (and custom X and Y) will suggest a new bank location!"));
        ...
        scriptFrame.add(createStartButton(disableBankCheckBox, bankOptions, miningCampOptions, customX, customY,
                rockOptions));
    ...
    private JButton createStartButton(JCheckBox disableBankCheckBox, JComboBox<String> bankOptions, JComboBox<String>
            miningCampOptions, JTextField customX, JTextField customY, JPanel rockOptions) {
        JButton startScriptButton = new JButton("Start");

        startScriptButton.addActionListener(e -> {
            ...
            scriptOptions.miningCampPoint =
                    customX.isEnabled() && customY.isEnabled() && isValidCustomCamp(customX, customY)
                            ...
                            : MiningCampPoint.valueOf((String) miningCampOptions.getSelectedItem()).getMapPoint();

            scriptOptions.rocksToMine = stream(Rock.values())
                    .filter(rock -> rock != EMPTY)
                    .filter(rock -> ((JCheckBox) rockOptions.getComponent(rock.ordinal() - 1)).isSelected())
                    .collect(toList());
    ...
    private static JComboBox<String> createBankOptions(JCheckBox disableBankCheckBox) {
        ...
        disableBankCheckBox.addItemListener(itemEvent ->
                bankOptions.setEnabled(itemEvent.getStateChange() == ItemEvent.DESELECTED));
        return bankOptions;
    }
    ...
    private static ActionListener suggestABank(JComboBox<String> bankOptions, JComboBox<String> miningCampOptions) {
        return e -> {
            MiningCampPoint miningCampPoint =
