...
public class PowercraftTalisman extends IdleScript {
	...
	public void scriptStart() {
		while (controller.isRunning()) {
			...
			while (controller.getInventoryItemCount(1299) >= 1 && !controller.isBatching()
					|| controller.getInventoryItemCount(1385) >= 1 && !controller.isBatching() && dropping == false) {
				...
				if (controller.getInventoryItemCount(1385) > 0 && controller.getInventoryItemCount(1299) < 1 && dropping == false) {
	...
	public void paintInterrupt() {
		if (controller != null) {
			...
			controller.drawString("@red@Drop timer: @yel@" + toRemainingString(dropTimer), 10, 64, 0xFFFFFF, 1);
