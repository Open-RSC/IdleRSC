...
public final class S_ArdMinePickup extends Script {
  ...
  private static final int[] items = {157, 158, 159, 160, 151};

  private static final int BANK_X = 551, BANK_Y = 612, DEST_X = 616, DEST_Y = 653;
  ...
  public S_ArdMinePickup(String ex) {
    //		super(ex);
  ...
  public void paint() {
    ...
    for (int id : gained.keySet()) {
      ...
      drawString(
          String.format(
              "%s gained: %s (%s/h)", getItemNameId(id), ifmt.format(count), per_hour(count)),
          x,
          y,
          2,
          0xFFFFFF);
  ...
  private boolean _insideBank() {
    return getX() <= 554 && getX() >= 551 && getY() <= 616 && getY() >= 609;
  }
}
