...
public class k_ShopBuyer2 extends Script {
  ...
  public void init(String s) {
    ...
    // If you want to use this script on only one npc to buy only x, y, z, fill this out and leave
    // parameters blank at runtime.
    // THINGS TO CHANGE
    veteran = false;
    npc_id = 149;
    item_ids = new int[] {31, 33};
        ...
        if (menu == -1) {
          ...
          for (int i = 0; i < questMenuCount(); ++i) {
            ...
            if (option.contains("sell")
                || option.contains("yes please")
                || option.contains("yes ok")
                || option.contains("see your wares")
                || option.contains("see what you've got")
                || option.contains("oh that sounds intersting")) {
              menu = i;
              return 10;
            }
          }
          System.out.println(
              "Menu option not supported. Please report the following to the script developer:");
  ...
  public void paint() {
    ...
    for (int item_id : item_ids) {
      ...
      if (purchased > 0) {
        drawString(
            getItemNameId(item_id) + ": " + purchased + " (" + perHour(purchased) + "/h)",
            x,
            y,
            2,
            0xFFFFFF);
  ...
  public void onServerMessage(String s) {
    ...
    if (s.contains("store")
        ...
        || s.contains("welcome")
        || s.contains("buy")
        || s.contains("how can i help")
        || s.contains("would you")) {
