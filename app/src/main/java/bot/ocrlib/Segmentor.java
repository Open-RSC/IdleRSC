...
final class Segmentor {
  ...
  private static final JoiningCriterionT smallRegionsCrowdingCriterion =
      new SmallRegionsCrowdingCriterion();
    ...
    for (i = 0; i < letters.size(); i++) {
      for (j = i + 1; j < letters.size(); j++) {
        if (joinCritFunc.joinCritFunc(letters.get(i), letters.get(j))) {
          rids.set(j, rids.get(j) < rids.get(i) ? rids.get(j) : rids.get(i));
          rids.set(i, j);
        }
      }
    }
  ...
  Region collapse_regions(ArrayList<Region> regs) {
    ...
    for (i = 0; i < regs.size(); i++) {
      if (regs.get(i).OffsetX() < minX) minX = regs.get(i).OffsetX();
      if (regs.get(i).OffsetY() < minY) minY = regs.get(i).OffsetY();
      ...
      for (i = 0; i < regs.get(k).Height(); i++) {
        for (j = 0; j < regs.get(k).Width(); j++) {
          if (regs.get(k).getPixel(j, i) != 0) {
            tmpReg.setPixel(
                j + regs.get(k).OffsetX() - minX,
                i + regs.get(k).OffsetY() - minY,
                regs.get(k).getPixel(j, i));
          }
        }
      }
    }
