 ...
 * Magic fatigue.
 *
 * <p>- Crandor isle - Wind bolts hob goblins till 99% - Kills Lesser demon, hops worlds if none
 * spawned. - Teles to ardounge to bank // market bank /// goes to karamja through the ship on ard
 * port. //// goes to volcano, pushes oddlooking wall and goes to crandor from there. (when banking,
 * doesnt sleep because many hostile NPCs on the route)
 *
 * <p>air staff! you should be able to change the spell this uses by editing "WIND_BOLT",
 * "STAFF_OF_AIR", "runes", "rune_req_counts"
 *
 * @author Storm
 */
public final class S_FatigueCrandor extends Script implements ActionListener {
  ...
  private static final int[] lewt = {399, 1277, 526, 527, 542};

  private final boolean[] banked = new boolean[lewt.length];
  private final int[] banked_counts = new int[lewt.length];

  private static final int[] runes = {41, 32, 42};

  private static final int[] rune_wd_counts = {500, 2, 2};

  private static final int[] rune_req_counts = {1, 2, 2};
  ...
  private final TextField tf_min_hp = new TextField(String.valueOf(min_hp)),
  ...
  public S_FatigueCrandor(String ex) {
    //        super(ex);
  ...
  public void paint() {
    ...
    if (gained > 0) {
      drawString(
          "Attack XP: " + int_format(gained) + " (" + per_hour(gained) + "/h)", x, y, font, white);
      y += 15;
    }
    gained = def - start_def;
    if (gained > 0) {
      drawString(
          "Defence XP: " + int_format(gained) + " (" + per_hour(gained) + "/h)", x, y, font, white);
      y += 15;
    }
    gained = str - start_str;
    if (gained > 0) {
      drawString(
          "Strength XP: " + int_format(gained) + " (" + per_hour(gained) + "/h)",
          x,
          y,
          font,
          white);
    ...
    if (gained > 0) {
      drawString(
          "Hits XP: " + int_format(gained) + " (" + per_hour(gained) + "/h)", x, y, font, white);
      y += 15;
    }
    gained = magic - start_magic;
    if (gained > 0) {
      drawString(
          "Magic XP: " + int_format(gained) + " (" + per_hour(gained) + "/h)", x, y, font, white);
      y += 15;
    }
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getActionCommand().equals("OK")) {
      ...
      brim_dock_to_gate =
          pw.calcPath(B_DOCK_APPROX_X, B_DOCK_APPROX_Y, GATE_APPROX_X, GATE_APPROX_Y);
      ladder_to_wall =
          pw.calcPath(LADDER_UP_APPROX_X, LADDER_UP_APPROX_Y, S_WALL_APPROX_X, S_WALL_APPROX_Y);
      wall_to_stairs =
          pw.calcPath(
              S_WALL_AFTER_APPROX_X, S_WALL_AFTER_APPROX_Y, STAIRS_UP_APPROX_X, STAIRS_UP_APPROX_Y);
    }
