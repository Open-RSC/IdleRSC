...
public class SAF_BloodThiever extends Script {
  ...
  public void init(String params) {
    ...
    String wait =
        (String)
            JOptionPane.showInputDialog(
                null,
                "Select wait time for NPC's in the way.",
                "Setup",
                JOptionPane.PLAIN_MESSAGE,
                null,
                waitOptions,
  ...
  private boolean no_stick() {
    if (last_x != getX() || last_y != getY()) stuck = false;
    ...
    if (last_x == getX() && last_y == getY() && stuck) {
      if (inCombat()) stuck = false;

      if (!inCombat()) {
        long secs = ((System.currentTimeMillis() - stuck_time) / 1000L);
        if (secs > 1) print("NPC has been blocking us for " + secs + " seconds.");

        if (secs == 1) print("NPC has been blocking us for " + secs + " second.");
