...
public class LocationWalker extends SeattaScript {
  ...
  private String chatCommandDestination;
  private boolean started = false;

  private final String filterPlaceholder = "Filter location names...";

  private final SpringLayout sl = new SpringLayout();
  private final JFrame frame = new JFrame("LocationWalker");
  private final JPanel panel = new JPanel();
  private final JButton startBtn = new JButton("Select a Location");
  private final JTextField filter = new JTextField(filterPlaceholder);
  private final String[] tableColumns = {"Location", "Coordinates"};
  ...
  private final JScrollPane locationScrollPane = new JScrollPane(table);
  ...
  private final Location[] hiddenLocations = {
    ...
    if (parameters.length > 0) {
      if (parameters.length == 2) {
        try {
          ...
          chatCommandDestination =
              Location.getDescriptionFromStandableTile(x, y).isEmpty()
                  ? String.format("(%s, %s)", x, y)
                  : Location.getDescriptionFromStandableTile(x, y);

          int width = Math.max(paintW, c.getStringWidth(chatCommandDestination, 1) + 8);
        ...
        destination =
            Arrays.stream(Location.values())
                .filter(l -> l.getDescription().equals(parameters[0]))
                .findFirst()
                .orElse(null);
      }
    }
    if (!c.isRunning()) return quit();
    if (destination == null) showUI();
    if (destination != null) {
      int width = Math.max(paintW, c.getStringWidth(destination.getDescription(), 1) + 8);
  ...
  public void showUI() {
    ...
    while (c.isRunning() && !started && frame.isVisible()) c.sleep(640);
  ...
  private void populateTable() {
    ...
    Arrays.stream(Location.values())
        ...
        .forEach(
            loc -> {
              if (!hidden.contains(loc) && loc.isWalkable() && loc.isWalkConditionMet())
                tableModel.addRow(
                    new String[] {
  ...
  private void tableSelectionChanged() {
    ...
    String selectedRow = String.valueOf(table.getValueAt(row, 0));
    ...
    if (selected != null) {
      // Show the Location's information
  ...
  private FocusListener getPlaceholderFocusListener() {
  ...
  public void paintInterrupt() {
    if (c != null) {
      ...
      } else if (chatCommandDestination != null) {
        paintBuilder.addSpacerRow(4);
        paintBuilder.addRow(rowBuilder.centeredSingleStringRow("Walking Towards: ", colorWhite, 1));
        paintBuilder.addRow(
            rowBuilder.centeredSingleStringRow(chatCommandDestination, colorPurple, 1));
      }
      paintBuilder.draw();
    }
  }
