...
public final class OgreRanger extends Script {
  ...
  public void paint() {
    ...
    y += 15;
    drawString(
        "Arrows spent: " + arrows + " (" + (xpStats[2] / arrows) + " XP/arrow)", x, y, 1, 0xFFFFFF);
    y += 15;
  ...
  public void onServerMessage(String s) {
    ...
    if (s.contains("near enough")
        || s.contains("an obstacle")
        || s.contains("been standing")
        || s.contains("close enough")) {
  ...
  public String getPlayerName(int local_index) { // storm
    ...
    return super.getPlayerName(local_index).replace((char) 160, ' ');
  }
