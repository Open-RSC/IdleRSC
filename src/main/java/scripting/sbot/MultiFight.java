...
public class MultiFight extends Script {
  ...
  public void showReport() {
    ...
    float hrlyKilled = 0;
    int totalXp =
        (GetExperience(0) - startAttExp)
            + (GetExperience(1) - startDefExp)
            + (GetExperience(2) - startStrExp);
    if (seconds > 0) hourlyExp = (((float) (totalXp) / (float) (seconds)) * 3600);
    Println(".-.-.-.-.-.--.-.-.-.-.--.-.-.-.--.-.-.-.-.--.-.-.-.-.--.-.-.-.--.-.-");
    ...
    Println(
        "You have killed "
            ...
            + ","
            + npcArr[3]
            + ")"); // "+npcArr[0]+","+npcArr[1]+","+npcArr[2]+","+npcArr[3]+")
    Println("You have slept " + slept + " times");
    if (seconds != 0) hrlyKilled = (((float) (killed) / (float) (seconds)) * 3600);
  ...
  public void runScript() {
    ...
    if (aw) {
      ...
      x = GetX();
      y = GetY();
    } else DisplayMessage("Anti Wander disabled", 3);
    time = TickCount();
    starttime = TickCount();
    Wait(500);
    while (Running() && sB() && !fT()) {
      ...
      if (fTCS() && Running()) tDS();
      if (!npc() && Running()) tI();
      if (fOB() && Running() && !sN()) buryBones();
      if (reportTime()) showReport();
    }
    if (!sB()) DisplayMessage("..You don't have a sleeping bag..", 3);
    else if (!fT()) DisplayMessage("Aww, you're going already?", 3);
    else if (fT() && sB()) DisplayMessage("W00t finished training!", 3);
