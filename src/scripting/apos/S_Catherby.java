...
public final class S_Catherby extends Script {
  ...
  public static final String[] combined_fish_spots = {
    "Shrimp/anchovies", "Tuna/swordfish", "Mackerel/cod/bass", "Lobster", "Shark", "Sardine/herring"
  };
  ...
  private static final int[] non_fish_net_loot = {
    779, 792, /* oysters */ 157, 158, 159, 160, 526, 527, 1277 /* casket stuff */
  };
  ...
  public void onServerMessage(String str) {
    ...
    if (str.contains("busy")) {
      ...
      if (cur_attempts >= 1000) _fishCsvOut();
      ...
      if (cur_cattempts >= 1000) _cookCsvOut();
      ...
      ++fishing_levels;
      System.out.printf(
          "Congrats on advancing your fishing level %d times since starting this script.\n",
          fishing_levels);
      System.out.println("Stats for your last level:");
      System.out.printf(
          "Successful attempts: %s (%s/h)\n",
      ...
      ++cooking_levels;
      System.out.printf(
          "Congrats on advancing your cooking level %d times since starting this script.\n",
          cooking_levels);
      System.out.println("Stats for your last level:");
      System.out.printf(
          "Successful attempts: %s (%s/h)\n",
  ...
  public void paint() {
    ...
    drawString("Catherby Script - @whi@" + stages.get(stage).toString(), x, y, 2, orangey);
    y += 15;
    drawString("Runtime: " + get_time_since(start_time), x + 10, y, 2, white);
    y += 15;
    if (have_stage(STAGE_FISH)) {
      drawString(
          String.format("Stats for current fishing level (%d gained)", fishing_levels),
          x,
          y,
          2,
          orangey);
      ...
      drawString(
          String.format(
              ...
              iformat.format(cur_fish_fails), per_hour(cur_fish_fails, fish_level_up_time)),
          x + 10,
          y,
          2,
          white);
      y += 15;
      drawString(
          "Fishing fail rate: " + (float) ((double) cur_fish_fails / (double) cur_fish_success),
          x + 10,
          y,
          2,
          white);
    ...
    if (have_stage(STAGE_COOK)) {
      drawString(
          String.format("Stats for current cooking level (%d gained)", cooking_levels),
          x,
          y,
          2,
          orangey);
      ...
      y += 15;
      drawString(
          "Cooking fail rate: " + (float) ((double) cur_cook_fails / (double) cur_cook_success),
          x + 10,
          y,
          2,
          white);
    ...
    if (fishing_levels > 0 || cooking_levels > 0) {
      ...
      if (fishing_levels > 0) {
        drawString(
            "Successful fishing attempts: " + iformat.format(total_fish_success),
            x + 10,
            y,
            2,
            white);
        y += 15;
        drawString(
            "Failed fishing attempts: " + iformat.format(total_fish_fails), x + 10, y, 2, white);
        y += 15;
      }
      if (cooking_levels > 0) {
        drawString(
            "Successful cooking attempts: " + iformat.format(total_cook_success),
            x + 10,
            y,
            2,
            white);
        y += 15;
        drawString(
            "Failed cooking attempts: " + iformat.format(total_cook_fails), x + 10, y, 2, white);
        y += 15;
      }
    }
  ...
  private int fish() {
    ...
    if (stages.size() > 1 && getInventoryCount() == MAX_INV_SIZE) {
      ...
      if (big_netting) {
        int index = getInventoryIndex(CASKET, UNOPENED_OYSTER);
        if (index != -1) {
          useItem(index);
          return random(1000, 1500);
        }
  ...
  private void create_frame() {
    ...
    final Checkbox uncert =
        new Checkbox("Uncert raw fish (sharks/bass only)", acquire_group, false);
    ...
    final Checkbox buy_raw =
        new Checkbox("Buy raw fish (Harry's Fishing Shack)", acquire_group, false);
    acquire_boxes.add(buy_raw);

    final Checkbox buy_cooked =
        new Checkbox("Buy cooked fish (Arhein's General Store)", acquire_group, false);
    ...
    final Checkbox sell_cooked =
        new Checkbox("Sell cooked fish (Arhein's General Store)", dispose_group, false);
    dispose_boxes.add(sell_cooked);
    sell_cooked.setEnabled(false);

    final Checkbox sell_raw =
        new Checkbox("Sell raw fish (Harry's Fishing Shack)", dispose_group, false);
    ...
    final Checkbox drop_raw_shrimp =
        new Checkbox("Special: Exclude shrimp / drop after fishing", false);

    final Checkbox drop_raw_sardine =
        new Checkbox("Special: Exclude sardine / drop after fishing", false);

    final CheckboxGroup big_net_specials = new CheckboxGroup();

    final Checkbox drop_raw_non_bass =
        new Checkbox(
            "Special: Exclude non-bass fish / drop after fishing", big_net_specials, false);

    final Checkbox sell_raw_non_bass =
        new Checkbox("Special: Sell raw non-bass fish", big_net_specials, false);

    final Checkbox sell_cooked_non_bass =
        new Checkbox("Special: Sell cooked non-bass fish", big_net_specials, false);
    sell_cooked_non_bass.setEnabled(false);

    final CheckboxGroup tuna_specials = new CheckboxGroup();

    final Checkbox drop_raw_tuna =
        new Checkbox("Special: Exclude tuna / drop after fishing", tuna_specials, false);

    final Checkbox sell_raw_tuna =
        new Checkbox("Special: Sell raw tuna, bank swordfish", tuna_specials, false);

    final Checkbox sell_cooked_tuna =
        new Checkbox("Special: Sell cooked tuna, bank swordfish", tuna_specials, false);
            ...
            switch (list.getSelectedItem()) {
              case "Anchovies/shrimp":
                ...
                if (drop_raw_shrimp.getState()) {
                  ...
                  raw_ids = new int[] {349, 351};
                  cooked_ids = new int[] {350, 352};
                  burnt_ids = new int[] {353};
                  discard_ids = new int[] {};
                }
                break;
              case "Herring/sardine":
                ...
                if (drop_raw_sardine.getState()) {
                  ...
                  raw_ids = new int[] {354, 361};
                  cooked_ids = new int[] {355, 362};
                  burnt_ids = new int[] {360, 365};
                  discard_ids = new int[] {};
                }
                break;
              ...
              case "Swordfish/tuna":
                ...
                if (drop_raw_tuna.getState()) {
                  ...
                  raw_ids = new int[] {366, 369};
                  cooked_ids = new int[] {367, 370};
                  burnt_ids = new int[] {368, 371};
                  discard_ids = new int[] {};
                }
                break;
              case "Sharks":
          ...
          public void actionPerformed(ActionEvent e) {
            ...
            if (deposit.getState()) {
              ...
              if (big_netting) {
                stages.add(
                    new Stage(
                        STAGE_DEPOSIT, calc_path(HARRY_SHOP_X, HARRY_SHOP_Y, BANK_X, BANK_Y)));
              }
              stages.add(new Stage(STAGE_SELL, calc_path(x, y, HARRY_SHOP_X, HARRY_SHOP_Y)));
              stages.get(0).path_to =
                  calc_path(
