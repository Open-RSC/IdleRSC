    ...
    public void scriptStart() {
        ...
        while (controller.isRunning()) {
            ...
            if (ingredients[5] != 0 &&
                ...
                if (controller.isItemInInventory(ingredients[4])) {//typically ignored if herb Identifier script is used (ideal)
                    cleanHerbs();
                }
                //mix unf potions
            ...
            if (ingredients[5] == 0 && ingredients[1] == 468){  //no unid in index, and has pestle in [1] slot
                //grind away
                grind();
            }
        }
...
//Ingredients: Full Vial[0], Clean Herb[1], Secondary[2], Empty Vial[3], Unid Herb[4], Unfinished Potion[5]
            ...
            if (ingredients[5] != 0) {
                //now we count up unif potions + math.min((clean+ dirty herbs),(Empty + full vials)) = # you can craft
                primaryIngredientTotalInBank =
                        controller.getBankItemCount(ingredients[5])  //unifs
                                + Math.min((controller.getBankItemCount(ingredients[1]) + controller.getBankItemCount(ingredients[4])),
                                (controller.getBankItemCount(ingredients[0]) + controller.getBankItemCount(ingredients[3])));
                primaryIngredientInBank = controller.getBankItemCount(ingredients[1]) + controller.getBankItemCount(ingredients[4]) + controller.getBankItemCount(ingredients[5]);
    ...
    public void setValuesFromGUI(JCheckBox unfinishedPotionCheckbox,JCheckBox stopCraftingSecondaryCheckbox) {
    ...
    public void paintInterrupt() {
        if(controller != null) {
            ...
            controller.drawString("@red@Coleslaw Potion Maker @cya@by Seatta and Kaila", 6, 21, 0xFFFFFF, 1);
            controller.drawString("@whi@__________________", 6, 21+3, 0xFFFFFF, 1);
            controller.drawString("@whi@"+primaryIngredientName+" Remaining: @gre@" + this.primaryIngredientInBank, 6, 21+17, 0xFFFFFF, 1);
            controller.drawString("@whi@"+secondaryIngredientName+" Remaining: @gre@" + this.secondaryIngredientTotalInBank, 6, 21+17+14, 0xFFFFFF, 1);
            controller.drawString("@whi@Total Vials Remaining: @gre@" + this.combinedVialsInBank, 6, 21+17+14+14, 0xFFFFFF, 1);
            if (ingredients[1] != 468) {
                controller.drawString("@whi@" + potion + "'s Made: @gre@" + String.format("%,d", made) + " @yel@(@gre@" + String.format("%,d", successPerHr) + "@yel@/@whi@hr@yel@)", 6, 21 + 17 + 28 + 14, 0xFFFFFF, 1);
            } else {
                controller.drawString("@whi@" + controller.getItemName(ingredients[2]) + " Ground: @gre@" + String.format("%,d", made) + " @yel@(@gre@" + String.format("%,d", successPerHr) + "@yel@/@whi@hr@yel@)", 6, 21 + 17 + 28 + 14, 0xFFFFFF, 1);
            }
            if(onlyMakeUnifsCycle == true) {
                controller.drawString("@whi@Time Remaining: " + toTimeToCompletion(made, this.primaryIngredientTotalInBank, startTime), 6, 21+17+42+14, 0xFFFFFF, 1);
            } else {
                controller.drawString("@whi@Time Remaining: " + toTimeToCompletion(made, Math.min(this.secondaryIngredientTotalInBank, this.primaryIngredientTotalInBank), startTime), 6, 21+17+42+14, 0xFFFFFF, 1);
            }
    ...
    public void parseValues() {
        ...
        controller.displayMessage("@red@REQUIRES Batch bars be toggle on in settings to work correctly!");
    ...
    public void setupGUI() {
        ...
        JComboBox<String> potionField = new JComboBox<String>(new String[] {"Attack", "Cure Poison", "Strength", "Runecraft",
                ...
                "Super Runecraft", "Weapon Poison", "Super Defense", "Ranging", "Magic", "Potion of Zamorak", "Potion of Saradomin", "Super Ranging", "Super Magic", "Ground Unicorn Horn", "Ground Blue Dragon Scale",
        ...
        startScriptButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (potionField.getSelectedItem().toString() != "" && potionField.getSelectedItem().toString() != "0") {
