...
public final class S_WineMaker extends Script {
  ...
  public S_WineMaker() {
    pw = new PathWalker(null);
  }

  public S_WineMaker(String ex) {
  ...
  public void paint() {
    ...
    y += 15;
    drawString("Runtime: " + get_time_since(start_time), 25, y, 1, gray);
    y += 15;
    drawString(
        String.format("Wine made: %s (%s/h)", iformat.format(wine_made), per_hour(wine_made)),
        25,
        y,
        1,
        gray);
    ...
    drawString(
        String.format("Cooking XP: %s (%s/h)", iformat.format(xp_gained), per_hour(xp_gained)),
        25,
        y,
        1,
        gray);
