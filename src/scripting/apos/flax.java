...
public class flax extends Script {
  ...
  private static final int[] ladder = {5, 684, 520};
  private static final int[] wheel = {121, 686, 1464};
  ...
  public void init(String p) {
    ...
    if (p.equals("")) print("Power Flax to Bowstring by Blood started!");
      ...
      // implemented
      // it
    } else print(p + " is an invalid parameter, starting with defaults.");
  ...
  public void onServerMessage(String s) {
    if (s.contains("make the flax into")) flax_made++;
    /*
  ...
  public void onChatMessage(String msg, String name, boolean pmod, boolean jmod) {
    ...
    if (pmod) {
      ...
      log(name + ", a player mod, said \"" + msg + "\", hopping.", "SystemLog");
      hop = true;
    }
    if (jmod) {
  ...
  public void onPrivateMessage(String msg, String name, boolean pmod, boolean jmod) {
    ...
    if (jmod) {
      ...
      log(name + " messaged us \"" + msg + "\", logging out.", "SystemLog");
      // AutoLogin.setAutoLogin(false);
      stopScript();
      logout();
    }
    if (msg.contains("many bowstrings")) sendPrivateMessage(flax_made + "", name);
    if (msg.contains("long have you been")) sendPrivateMessage(getRunTime(), name);
    if (msg.contains("crafting level"))
      sendPrivateMessage(getLevel(skillName("crafting")) + "", name);
  }

  public void paint() {
    double xp_til_next_level =
        next_level[getLevel(skillName("crafting"))] - getXpForLevel(skillName("crafting"));
    double xp_between_levels =
        next_level[getLevel(skillName("crafting"))]
  ...
  private long getXpH() {
    try {
      long xph =
          (((getXpForLevel(skillName("crafting")) - start_xp) * 60) * 60)
