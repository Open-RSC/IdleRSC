...
public final class SAF_GrandExchange extends Script implements ActionListener {
  ...
  private final Choice combat_choice = new Choice(), sorb_choice = new Choice();
  private final TextField tf_chat_delay = new TextField(String.valueOf(chatDelay)),
    ...
    if (banking) {
      ...
      if (isInTradeOffer()) {
        if ((System.currentTimeMillis() - time) / 1000L > 60) {
          System.out.println(
              "Declined trade because user took too long - "
        ...
        if (merchantMode == 0) {
          ...
          if (hasOtherTraded(10, 1) && (getRemoteTradeItemStack(0) % itemPrice) == 0) {
            ...
            for (int i = 0; i < itemID.size(); ++i) {
              if (isItemStackableId(itemID.get(i))) {
                // Find out our max amount we can sell to them and if they offer more than we have
                // then adjust itemAmount variable
                ...
                // to trade the amount they offered and if not then we adjust the itemAmount and
                // tell the trade
              ...
              // If we have offered less than they want to buy then we need to add more items and
              // add to our internal total
            ...
            // If by some miracle we get matching offeredAmounts and they equal itemAmount then we
            // accept the trade
          ...
          // This is what we do when there are no gold coins in the traders offer
            ...
            // If we have yet to offer them anything then we want to let them know the price and
            // that they must enter exact coins
        ...
        // If merchantMode != 0 then we are in buying mode
          ...
          if (getTheirTradedItemCount() > 0) {
            ...
            for (int i = 0; i < itemID.size(); ++i) {
              ...
              if (isItemStackableId(itemID.get(i))) {
                // If it is more than the amount of gold coins we have then we need to set
                // itemAmount to our maxStack
            ...
            if (testEqual(offeredAmount)) {
              // If the coinsOffered is less than or rqual to itemAmount * itemPrice then we need to
              // add some to make it equal
