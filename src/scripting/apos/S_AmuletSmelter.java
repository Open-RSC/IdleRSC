...
public final class S_AmuletSmelter extends Script implements ActionListener {
  ...
  private static final int[] gems = {-1, 164, 163, 162, 161, 523};

  private static final int[] unstrung = {296, 297, 298, 299, 300, 524};

  private static final String[] loc_names = {"Al Kharid", "Ardougne", "Falador"};
  ...
  public S_AmuletSmelter(String ex) {
    //        super(ex);
  ...
  public int main() {
    ...
    if (isAtApproxCoords(furnace_point.x, furnace_point.y, 5)) {
      ...
      } else if (getInventoryIndex(GOLD_BAR) != -1
          && (gem_id == -1 || getInventoryIndex(gem_id) != -1)) {
  ...
  public void paint() {
    ...
    for (int i = 0; i < amulet_names.length; ++i) {
      ...
      drawString("Banked " + amulet_names[i] + " amulets: " + banked_counts[i], 25, y, 1, 0xFFFFFF);
      y += 15;
    }
  }
