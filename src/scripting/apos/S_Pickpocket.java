...
public final class S_Pickpocket extends Script implements ActionListener, ItemListener {
  ...
  static {
    ...
    map_npcs.put("Rogues (actually Watchmen)", new int[] {342, 574});
    map_npcs.put("Guards (level 40)", new int[] {65, 321, 376, 385});
    ...
    map_food.put("Shrimp and Bread", new int[] {350, 138});
  ...
  private static final int[] ids_bank = {
    COINS, 31, 32, 33, 34, 35, 36, 37, 38, 40, 41, 41, 42, 46, 619, 152, 142, 612, 619, 161, DSQ
  };
  ...
  public void init(String params) {
    ...
    if (frame == null) {
      ...
      frame.add(
          new Label("Banking is supported from most ground level locations.", Label.CENTER),
          BorderLayout.CENTER);
      frame.add(new Label("Start this script at the NPCs.", Label.CENTER), BorderLayout.SOUTH);
  ...
  public int main() {
    ...
    if ((item[1] == getX() && item[2] == getY())
      ...
      if (getInventoryCount() < MAX_INV_SIZE
          || (getInventoryIndex(item[0]) != -1 && isItemStackableId(item[0]))) {
  ...
  public void paint() {
    ...
    if (levels_gained > 0) {
      ...
      y += 15;
      drawString("Successful attempts: " + iformat.format(total_success), x + 10, y, 1, white);
      y += 15;
      drawString("Failed attempts: " + iformat.format(total_fails), x + 10, y, 1, white);
    }
    ...
    if (withdraw_food != 0) {
      drawString(
          String.format(
              "%s food withdrawn (%s trips)",
              iformat.format(total_withdraw), iformat.format(total_withdraw / withdraw_food)),
          x + 10,
          y,
          1,
          white);
    ...
    for (int i = 0; i < len; ++i) {
      ...
      if (count <= 0) continue;
      drawString(
          String.format("%s %s", iformat.format(count), getItemNameId(ids_bank[i])),
          x + 10,
          y,
          1,
          white);
  ...
  private boolean shouldBank() {
    switch (ch_bank.getSelectedIndex()) {
      ...
      default:
        return getFoodSlot() == -1 || getInventoryCount() == MAX_INV_SIZE;
    }
  }
